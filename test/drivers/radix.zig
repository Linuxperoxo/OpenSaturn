...
pub const Allocators: type = struct {
    pub const Lines: type = struct {
        const SOA: type = @import("SOA.zig");
        const Allocator_T: type = SOA.buildObjAllocator(
            [4]?*anyopaque,
            64,
            SOA.CacheSize_T.auto,
            null,
            null,
            null,
            SOA.Align_T.in8,
            SOA.Optimize_T.optimized,
            SOA.Cache_T.PrioritizeHits,
            SOA.CacheTry_T.Insistent,
        );
        ...
        pub fn free(obj: *anyopaque) AllocatorErr_T!void {
    ...
    pub const Levels: type = struct {
        ...
        pub fn alloc(T: type) AllocatorErr_T!*T {
            ...
            const level: *T = @alignCast(@ptrCast(@call(.always_inline, Allocator_T.alloc, .{
                &allocator
            }) catch |err| return err));
            if(@hasField(T, "map")) level.map = 0;
            return level;
        }
