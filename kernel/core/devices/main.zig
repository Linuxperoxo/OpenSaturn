...
pub inline fn dev_info(major: types.Major_T, comptime field: enum { name, type }) @FieldType(types.Dev_T, @tagName(field)) {
...
pub noinline fn dev_add(major: types.Major_T, dev: *const types.Dev_T) types.DevErr_T!void {
    ...
    if(!builtin.is_test) {
        mem.zeroe_mem(&majors[major].minors);
    } else {
        for(1..16) |i| {
...
pub inline fn dev_rm(major: types.Major_T) types.DevErr_T!void {
    bitmap &= ~major;
}
