...
pub fn mount(src: []const u8, path: []const u8, fs_name: []const u8) VfsErr_T!void {
    ...
    const fs_struct: *Fs_T = fs.schfs(fs_name)
        catch return VfsErr_T.NoNFound;

    const sblock: *const SuperBlock_T = fs_struct.mount(src)
...
pub fn umount(path: []const u8) VfsErr_T!void {
    const dentry_umount: *Dentry_T = try aux.resolve_path(path, &root);

    if(dentry_umount.d_sblock == null)
        return VfsErr_T.NothingToUmount;
...
pub noinline fn create(
    ...
    const dentry_parent: *Dentry_T = aux.resolve_path(parent, &root)
...
pub noinline fn chmod(path: []const u8, mode: mode_T,) VfsErr_T!void {
    const dentry_chmod: *Dentry_T = try aux.resolve_path(path, &root);
...
pub noinline fn chown(
    ...
    const dentry_chown: *Dentry_T = try aux.resolve_path(path, &root);
...
pub noinline fn read(path: []const u8, offset: usize) VfsErr_T![]u8 {
    const dentry_read: *Dentry_T = try aux.resolve_path(path, &root);
...
pub noinline fn write(path: []const u8, src: []const u8, offset: usize) VfsErr_T!void {
    const dentry_write: *Dentry_T = try aux.resolve_path(path, &root);
...
pub noinline fn unlink(path: []const u8) VfsErr_T!void {
    const dentry_unlink: *Dentry_T = try aux.resolve_path(path, &root);
...
pub noinline fn touch(path: []const u8) VfsErr_T!void {
    _ = try aux.resolve_path(path, &root);
}
