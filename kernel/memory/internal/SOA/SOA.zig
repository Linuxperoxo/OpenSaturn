...
pub fn buildObjAllocator(
    ...
    return struct {
        ...
        cache: if(optimize.type == .linear) void else [CacheElementSize]?BitMaxIPool = [_]?BitMaxIPool {
            null
        } ** CacheElementSize,
...
const allocators = r: {
    var allocs: [96]type = undefined;
    const optmizeAlloc: [3]Optimize_T.OptimizeRange_T = .{
        .huge, .large, .small
    };
    const cacheSync: [4]Cache_T.CacheSync_T = .{
        .burning, .chilled, .frozen, .heated
    };
...
test "SOA Continuos Alloc" {
    inline for(0..allocators.len) |a| {
        ...
        _ = SOAAllocator_T.alloc(
            ...
            SOAAllocator_T.err_T.OutOfMemory => return,
            else => return err,
        };
        return TestErr_T.TestUnreachableCode;
    }
}
